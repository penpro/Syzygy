...
| Remote provider key leaks into webview/project/log/MCP | Rust-only unwired transport uses a redacted secret wrapper; fake-server tests prove response errors do not echo secret or body canaries | OS credential vault, memory-zeroing review, webview/log/export canary scan, and platform tests remain release gates |
