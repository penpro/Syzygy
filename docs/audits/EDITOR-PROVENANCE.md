...
| `frontend/src/workspace/researchEventAttribution.ts` | Penumbra original | Vote, annotation, label/assignment, and immutable policy-version adoption layer that locates/hashes exact committed product/MCP events, rejects cross-author claims, publishes best-effort registered-device attribution, and reports explicit unsigned fallback without rolling back research state |
