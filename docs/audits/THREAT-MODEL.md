...
| Contract scaffolding is mistaken for a working feature | MCP separates adapter conformance, `fake-network-certified-unwired` task runtime, `runtime-boundary-unwired` aggregate status, and truly `contract-only` runners | Update status only with the named end-to-end evidence; never collapse internal wiring into availability |
...
| Internal provider runtime bypasses human disclosure | Transport rejects missing/mismatched approval; runtime test records denial without network contact; generation is absent from the Tauri handler | The native disclosure surface, approval lifetime/content binding, and cross-language record validation must land before registration |
