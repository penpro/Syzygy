...
| Fragmented or future provider stream corrupts research evidence | Incremental OpenAI decoder tests split UTF-8 at every byte, join multiline data, surface unknown types, order usage before finish, sanitize errors, and fail malformed/mismatched/oversized/truncated frames | Network cancellation, retry/duplicate event semantics, tool arguments, and adversarial long-stream fuzzing remain open |
