...
export const NETWORK_BOUNDARY_ITEMS: readonly NetworkBoundaryItem[] = [
  ...
  },
  {
    ...
    defaultState: 'Off until you create or accept a legacy, role-specific bearer, or device-bound invitation',
    ...
    data: 'The relay receives project updates and ephemeral presence. Bundled hosting persists bounded document-sync updates but never awareness. Managed invitations carry a role-specific capability; a device-bound admin can send separately signed status, issue, rotate, or revoke actions with public member/device metadata, and receives each new capability once. Legacy rooms use one read/edit room key. These authorize relay access but do not authenticate self-reported identities, expiry trusts the relay host clock, and the relay log is not a backup.',
  },
  {
