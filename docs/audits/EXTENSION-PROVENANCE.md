...
| `frontend/src-tauri/src/model_provider.rs` | repository commit | Penumbra original / repository MIT | Disclosure-gated OpenAI Responses request/stream and Anthropic Messages one-shot construction, provider-neutral normalization, bounded deadlines/bytes, and caller-controlled cancellation. |
