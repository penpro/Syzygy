...
| Remote provider silently stores research or transmits to an unsafe endpoint | OpenAI and Gemini request fixtures force `store:false`; OpenAI, Anthropic, and Gemini requests require matching content disclosure and accept only HTTPS or literal loopback HTTP | Provider-specific streaming/tools plus xAI and custom adapters need equivalent wire inspection; every provider policy must be rechecked at release |
