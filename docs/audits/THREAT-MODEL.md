...
| Contract scaffolding is mistaken for a working feature | MCP separates adapter conformance, `native-disclosure-command` task runtime, `native-disclosure-command-no-product-ui` aggregate status, and truly `contract-only` runners | The registered command still lacks a product caller and live-provider evidence; update availability only with the named end-to-end evidence |
...
| Provider caller forges or bypasses human disclosure | `ProviderTaskRequest` has no approval field; registered generation always opens a Rust-owned native dialog; denial test proves no vault read or network; transport still requires matching one-use approval | Categories are caller-described rather than derived from a frozen task snapshot; bind category derivation and input hash to domain orchestration before product availability; packaged UI must exercise the real OS dialog |
