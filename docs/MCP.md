...
| `inspect_plugin_workspace` | no | Returns bounded local installed-version, active-package, and shared plugin-review metadata without component, proposal, or signature bodies, plus exact document/research revisions and a no-automatic-mutation disclosure; it cannot change package lifecycle |
