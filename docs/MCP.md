...
| `inspect_plugin_workspace` | no | Returns bounded session-package and shared plugin-review metadata without component or proposal bodies, plus exact document/research revisions and a no-automatic-mutation disclosure |
| `run_loaded_plugin` | shared proposal events | Runs only an exact package already loaded by the person in the current GUI under document/research revision guards; publishes a bounded proposal batch, reports each proposal's signed-device or explicit unsigned attribution, and never applies draft text |
...
5. all 48 semantic tools are discoverable and route to their intended live operation, including bounded Drive project catalog/share/join, exact shared-title rename/retention guards, exact scenario sibling reconciliation, suggestion proposal/decision, plugin inspect/run, and adversarial archive/decision actions;
