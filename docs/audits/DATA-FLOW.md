...
| Native-gated provider task | Typed frontend call, currently with no product caller | Webview request → Rust native disclosure → OS vault → fixed provider HTTPS route → content-free record | Selected prompt leaves only after native **Send once**; hashes/source IDs/status/usage return | Key in OS vault; call/record persistence belongs to future workflow | No caller approval field; denial precedes vault/network; bounded categories/model; fixed route; timeout/cancellation; MCP has no invoke authority |
