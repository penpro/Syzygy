...
async function proveStdioContract() {
  ...
  if (!Array.isArray(tools) || tools.length < 29) throw new Error('MCP tool discovery is incomplete')
  ...
  if (contracts?.implementationStatus?.providerBatchAuthorization !== 'native-scoped-authorizer-no-product-executor') throw new Error('provider batch authorization status is inaccurate')
  if (contracts?.implementationStatus?.providerBatchReservation !== 'internal-atomic-reservation-no-executor') throw new Error('provider batch reservation status is inaccurate')
