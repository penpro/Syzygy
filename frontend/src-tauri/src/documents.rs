...
pub fn compile_typst(
    ...
    let work = std::env::temp_dir().join("syzygy-typst");
...
pub fn save_document(
    ...
    if !path_allowed(
        &granted.0.lock().unwrap_or_else(|e| e.into_inner()),
        &folder,
    ) {
...
pub fn list_documents(granted: tauri::State<Granted>, folder: String) -> Vec<String> {
    const DOC_EXTS: &[&str] = &[
        "typ", "md", "markdown", "txt", "text", "html", "htm", "css", "js", "ts", "jsx", "tsx",
        "json", "csv", "xml", "yaml", "yml", "java", "py", "rs", "go", "c", "cpp", "h", "sh",
        "sql",
    ];
    ...
    if let Ok(entries) = std::fs::read_dir(&folder) {
        for e in entries.flatten() {
            let p = e.path();
            let is_doc = p.extension().map_or(false, |x| {
                DOC_EXTS.contains(&x.to_string_lossy().to_lowercase().as_str())
            });
            if is_doc {
                let name = p
                    .file_name()
...
pub fn save_text_document(
    ...
    let ext_clean: String = ext
        ...
        .collect::<String>()
        .to_lowercase();
    let ext_clean = if ext_clean.is_empty() {
        "txt".to_string()
    } else {
        ext_clean
    };
...
pub fn write_temp_file(name: String, content: String) -> Result<String, String> {
    ...
    let dir = std::env::temp_dir().join("syzygy-docs");
...
pub fn read_text_file(granted: tauri::State<Granted>, path: String) -> Result<String, String> {
    if !path_allowed(&granted.0.lock().unwrap_or_else(|e| e.into_inner()), &path) {
        return Err(
            "That file isn't in a folder you've opened — use the Open button to pick it.".into(),
        );
    }
...
mod tests {
    ...
    fn path_allowed_scopes_to_granted_dirs() {
        ...
        assert!(path_allowed(
            &granted,
            &base.join("a.txt").to_string_lossy()
        ));
        // a not-yet-created file inside a granted dir (parent resolves)
        assert!(path_allowed(
            &granted,
            &base.join("new.pdf").to_string_lossy()
        ));
        // outside any granted dir
        assert!(!path_allowed(
            &granted,
            &std::env::temp_dir()
                .join("aph-outside.txt")
                .to_string_lossy()
        ));
