...
| `frontend/src/workspace/researchEventAttribution.ts` | Penumbra original | First domain adoption layer that locates/hashes a committed MCP scenario-vote event, publishes best-effort registered-device attribution, and reports explicit unsigned fallback without rolling back research state |
