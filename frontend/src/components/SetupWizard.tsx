 ...
 *  for a close affordance. First run instead offers an explicit engine-free path. */
  ...
  const go = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 120; i++) {
        ...
        if ((await getEngineStatus(baseUrl)) === 'ready') {
          ready = true
          break
        }
      }
      if (!ready) throw new Error('The local model did not become ready. Try starting it again.')
      updateSettings({ model: model.filename, localAiEnabled: true })
    ...
    >
      ...
      >
        ...
        <p className="muted" style={{ marginTop: 0 }}>
          Choose a local model, or continue without one and use projects or remote API providers.{' '}
          {vramGb
        ...
        {phase === 'choose' && (
          <>
            ...
            {onSkip && (
              <button className="btn ghost" onClick={onSkip} style={{ marginTop: 8, width: '100%' }}>
