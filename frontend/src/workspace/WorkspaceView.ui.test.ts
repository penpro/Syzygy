...
describe('workspace collaboration entry points', () => {
  ...
  it('offers the app-managed relay with role and legacy bearer disclosure', () => {
    ...
    expect(host).toContain('a different surviving')
    expect(host).toContain('recovery requires')
    expect(host).toContain('does not export or escrow installation private keys')
