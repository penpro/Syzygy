...
| Native adversarial batch authorization | Future workflow requests one panel consent | Real question/frozen source scope + exact remote routes/budgets → Rust validation → native dialog → random capability | Route/model/count/source identity and derived content categories; no research text in dialog/status | Rust process memory, maximum 30 minutes | Denial stores nothing; random 256-bit ID; exact summed ceilings; bounded active registry; content-free status; explicit revoke; no vault/network/call consumer |
