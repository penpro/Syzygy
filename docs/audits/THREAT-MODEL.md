...
| Remote provider silently stores research or transmits to an unsafe endpoint | OpenAI request fixture forces `store:false`; OpenAI and Anthropic requests require matching content disclosure and accept only HTTPS or literal loopback HTTP | Provider-specific streaming/tools and Gemini, xAI, and custom adapters need equivalent wire inspection; every provider policy must be rechecked at release |
