...
| `frontend/src/workspace/scenarioModel.ts` | Penumbra original | Stable lifecycle scenarios, ordered multi-turn CRDT content, attributed immutable revisions/edits, exact-current turn guards with idempotent retry, branch inspection, fail-closed peer-collision handling, and atomic lossless snapshot import |
...
| `frontend/src/workspace/ScenarioTurnWorkspace.tsx` | Penumbra original | Manual no-AI ordered-turn creation, exact-current immutable revision editing, visible stale-draft recovery, integrity lockout, and bounded conversation/lineage rendering |
| `frontend/src/workspace/ScenarioTurnWorkspace.ui.test.tsx` | Penumbra original | Headless product contract for attribution, idempotence, stale and hostile zero-write behavior, disconnected sibling convergence, accessible state, and paging bounds |
| `frontend/src/workspace/ScenarioCollaborationPanel.tsx` | Penumbra original | Shared scenario/turn note and flag lifecycle plus project-label assignment UI with exact-parent conflict refusal, write-time integrity checks, and bounded paging |
| `frontend/src/workspace/ScenarioCollaborationPanel.ui.test.tsx` | Penumbra original | Headless product contract for annotation lifecycle, label assignment/rename, integrity lockout, identity disclosure, and paging |
...
| `frontend/src/workspace/scenarioAutomation.ts` | Penumbra original | Live-project identity, graph validation, bounded scenario background/turn-head discovery, single current/named/indexed revision body readback, and monotonic research-revision guards for MCP scenario mutations |
...
| `frontend/src/automationBridge.ts` and `frontend/src-tauri/src/{automation,mcp}.rs` | Penumbra original | Live semantic dispatcher, authenticated loopback bridge, bounded explicit scenario-index and selected-revision content reads, and stdio MCP protocol implementation |
