...
describe('NetworkBoundarySummary', () => {
  it('states every optional network boundary and keeps local, remote, Drive, and control paths distinct', () => {
    ...
    expect(html).toContain('The WS/WSS endpoint in the invitation')
    expect(html).toContain('bundled hosting listens only on the private address you configure')
    expect(html).toContain('persists bounded document-sync updates but never awareness')
    expect(html).toContain('Managed invitations carry a role-specific capability; a device-bound admin can send separately signed status, issue, rotate, or revoke actions')
    expect(html).toContain('receives each new capability once')
    expect(html).toContain('Legacy rooms use one read/edit room key')
    expect(html).toContain('do not authenticate self-reported identities')
    expect(html).toContain('expiry trusts the relay host clock')
