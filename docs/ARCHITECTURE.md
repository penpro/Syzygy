...
| `model_provider.rs` | Rust-owned remote-model HTTP/normalization boundary. OpenAI Responses, Anthropic Messages, and Gemini Interactions one-shot/SSE plus xAI Responses one-shot wire contracts have fake-server evidence with bounded controls and sanitized normalization. |
...
| `provider_stream.rs` | Incremental provider SSE normalization. OpenAI, Anthropic, and Gemini decoders handle fragmented frames, text/usage/finish lifecycles, unknown future events, sanitized provider errors, and bounded malformed/truncated input. Anthropic and Gemini private-thinking/signature/tool bodies are not retained by normalized streams. |
