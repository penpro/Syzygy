...
fn call_tool(name: &str, arguments: Value, live: &LiveCall<'_>) -> Value {
    let operation = match name {
        ...
        "inspect_plugin_workspace" => live("plugin.inspectWorkspace", json!({})),
        "run_loaded_plugin" => live("plugin.runLoaded", arguments),
        "apply_accepted_plugin_review" => live("plugin.applyAccepted", arguments),
...
fn tool_definitions() -> Vec<Value> {
    vec![
        ...
        tool(
            ...
            "Inspect content-minimized metadata for publisher-signed versions installed locally, packages active in this running app, and bounded shared plugin review. Proposal content, component bytes, and signature bodies are omitted. This cannot install, enable, upgrade, roll back, remove, execute, decide, apply, or grant authority.",
            object_schema(&[], &[]),
        ),
        tool(
            "run_loaded_plugin",
            "Run one exact contribution from a package already active in this app session, including a signed installed package restored after re-verification. Requires exact document and research revisions. The zero-import component receives the bounded current draft only when project.read was requested; any valid output becomes shared pending review. Network, Drive, model, filesystem, installation lifecycle, and automatic draft mutation remain unavailable to MCP.",
            object_schema(
                &[
                    ("packageId", string_schema("Exact in-memory package ID from inspect_plugin_workspace.")),
                    ("contributionId", string_schema("Exact contribution ID from inspect_plugin_workspace.")),
                    ("expectedDocumentRevision", string_schema("Exact revision from read_active_project.")),
                    ("expectedResearchRevision", string_schema("Exact revision from inspect_plugin_workspace.")),
                ],
                &["packageId", "contributionId", "expectedDocumentRevision", "expectedResearchRevision"],
            ),
        ),
        tool(
            ...
            object_schema(
                &[
                    ...
                    ("expectedProposalEventId", string_schema("Exact proposal event ID from inspect_plugin_workspace.")),
                    ("expectedDecisionEventId", string_schema("Exact accepted decision event ID from the shared review.")),
                    ("expectedDocumentRevision", string_schema("Exact revision from read_active_project; must also equal the proposal's source revision.")),
                    ("expectedResearchRevision", string_schema("Exact revision from inspect_plugin_workspace.")),
                    ("confirmFullReplacement", json!({ "type": "boolean", "description": "Must be true only when explicitly confirming a retained replace operation; false for append." })),
                ],
                &[
        ...
        tool(
            ...
            "Explicitly archive one completed transient job into the active collaborative project. This stores the full question, selected source excerpts, validated results, baselines, and content-free provider provenance as shared project content, so Drive-backed projects can synchronize it to collaborators. Requires the exact research revision, then attempts a best-effort exact-archive registered-device signature. The response reports signed or explicitly unsigned attribution plus the post-attribution revision; signing failure never rolls back the archive and never authenticates a person. It never edits the draft.",
            object_schema(
                &[
        ...
        tool(
            ...
            "Append an immutable human accept/reject event to one saved adversarial review. Requires the exact archive hash, exact current research revision, and exact current decision event when revising a prior decision. Stores optional notes in shared history but omits their body from bounded inspection and the tool response, then attempts a best-effort signature over the exact retained decision. The response reports signed or explicitly unsigned attribution plus the post-attribution revision; signing failure never rolls back the decision and never authenticates a person. Never edits the draft.",
            object_schema(
                &[
...
mod tests {
    ...
    fn negotiates_current_protocol_and_advertises_tools() {
        ...
        assert!(names.contains(&"inspect_plugin_workspace"));
        assert!(names.contains(&"run_loaded_plugin"));
        assert!(names.contains(&"apply_accepted_plugin_review"));
        ...
        for tool_name in ["save_adversarial_review", "decide_adversarial_review"] {
            let review_tool = tools["result"]["tools"]
                ...
                .unwrap();
            let description = review_tool["description"].as_str().unwrap();
            assert!(description.contains("signed or explicitly unsigned attribution"));
            assert!(description.contains("post-attribution revision"));
            assert!(description.contains("never authenticates a person"));
        }
    ...
    #[test]
    fn routes_loaded_zero_authority_plugin_inspection_and_execution() {
        let inspection = dispatch_message(
            &json!({
                "jsonrpc": "2.0",
                "id": "plugin-inspect",
                "method": "tools/call",
                "params": { "name": "inspect_plugin_workspace", "arguments": {} }
            }),
            &fake_live,
        )
        .unwrap();
        ...
        let arguments = json!({
            "packageId": "org.example.fixture@1.0.0#0123456789abcdef",
            "contributionId": "review",
            "expectedDocumentRevision": "document-revision",
            "expectedResearchRevision": "research-revision",
            "confirmFullReplacement": false
        });
        let execution = dispatch_message(
            &json!({
                "jsonrpc": "2.0",
                "id": "plugin-run",
                "method": "tools/call",
                "params": { "name": "run_loaded_plugin", "arguments": arguments }
            }),
            &fake_live,
        )
        .unwrap();
        assert_eq!(
            execution["result"]["structuredContent"]["method"],
            "plugin.runLoaded"
        );
        assert_eq!(
            execution["result"]["structuredContent"]["params"],
            arguments
        );
        ...
        let apply_arguments = json!({
            ...
            "expectedDecisionEventId": "decision-1",
            "expectedDocumentRevision": "document-revision",
            "expectedResearchRevision": "research-revision"
        });
        let application = dispatch_message(
            &json!({
                "jsonrpc": "2.0",
                "id": "plugin-apply",
                "method": "tools/call",
                "params": { "name": "apply_accepted_plugin_review", "arguments": apply_arguments }
            }),
            &fake_live,
        )
        .unwrap();
        assert_eq!(
            application["result"]["structuredContent"]["method"],
            "plugin.applyAccepted"
        );
        assert_eq!(
            application["result"]["structuredContent"]["params"],
            apply_arguments
        );
    }
