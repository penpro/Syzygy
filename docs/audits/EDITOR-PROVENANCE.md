...
| `frontend/src/workspace/researchEventAttribution.ts` | Penumbra original | Vote, annotation, and label/assignment adoption layer that locates/hashes exact committed product/MCP events, publishes best-effort registered-device attribution, and reports explicit unsigned fallback without rolling back research state |
