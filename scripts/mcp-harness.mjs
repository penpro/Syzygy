...
async function proveStdioContract() {
  ...
  if (!Array.isArray(tools) || tools.length < 42) throw new Error('MCP tool discovery is incomplete')
  ...
  if (!tools.some((tool) => tool.name === 'list_shared_projects')) throw new Error('shared-project catalog tool is missing')
  if (!tools.some((tool) => tool.name === 'share_active_project')) throw new Error('project share tool is missing')
  if (!tools.some((tool) => tool.name === 'join_shared_project')) throw new Error('project join tool is missing')
  const compactDriveProject = tools.find((tool) => tool.name === 'compact_drive_project')
  if (!compactDriveProject) throw new Error('Drive project compaction tool is missing')
  ...
  const startRepairInspection = tools.find((tool) => tool.name === 'start_drive_title_repair_inspection')
  const inspectRepairJob = tools.find((tool) => tool.name === 'inspect_drive_title_repair_job')
  const startRepair = tools.find((tool) => tool.name === 'start_drive_title_repair')
  if (!startRepairInspection || !inspectRepairJob || !startRepair) {
    throw new Error('Drive title-repair job tools are incomplete')
  }
  if (startRepairInspection.inputSchema?.additionalProperties !== false ||
    inspectRepairJob.inputSchema?.additionalProperties !== false ||
    startRepair.inputSchema?.additionalProperties !== false) {
    throw new Error('Drive title-repair job schemas are not strict')
  }
  if (!inspectRepairJob.inputSchema?.required?.includes('jobId') ||
    !startRepair.inputSchema?.required?.includes('expectedRepairRevision') ||
    startRepair.inputSchema?.properties?.expectedRepairRevision?.minLength !== 64 ||
    startRepair.inputSchema?.properties?.expectedRepairRevision?.maxLength !== 64) {
    throw new Error('Drive title-repair job guards are incomplete')
  }
  ...
  const inspectAdversarial = tools.find((tool) => tool.name === 'inspect_adversarial_review')
  const cancelAdversarial = tools.find((tool) => tool.name === 'cancel_adversarial_review')
  const saveAdversarial = tools.find((tool) => tool.name === 'save_adversarial_review')
  const decideAdversarial = tools.find((tool) => tool.name === 'decide_adversarial_review')
  if (!startAdversarial) throw new Error('adversarial review start tool is missing')
  if (!inspectAdversarial) throw new Error('adversarial review inspection tool is missing')
  if (!cancelAdversarial) throw new Error('adversarial review cancellation tool is missing')
  if (!saveAdversarial) throw new Error('adversarial review save tool is missing')
  if (!decideAdversarial) throw new Error('adversarial review decision tool is missing')
  if (startAdversarial.inputSchema?.additionalProperties !== false) throw new Error('adversarial review start schema is not strict')
  if (!startAdversarial.inputSchema?.required?.includes('expectedDocumentRevision')) throw new Error('adversarial review start omits the document revision guard')
  if (!startAdversarial.inputSchema?.required?.includes('sourceBlockIndexes')) throw new Error('adversarial review start omits source selection')
  if (inspectAdversarial.inputSchema?.additionalProperties !== false || cancelAdversarial.inputSchema?.additionalProperties !== false) throw new Error('adversarial review job schemas are not strict')
  if (saveAdversarial.inputSchema?.additionalProperties !== false || decideAdversarial.inputSchema?.additionalProperties !== false) throw new Error('adversarial review history schemas are not strict')
  if (!saveAdversarial.inputSchema?.required?.includes('expectedResearchRevision')) throw new Error('adversarial review save omits the research revision guard')
  if (!decideAdversarial.inputSchema?.required?.includes('recordSha256')) throw new Error('adversarial review decision omits the archive hash guard')
  ...
  if (contracts?.implementationStatus?.providerBatchAuthorization !== 'native-content-bound-call-graph-authorizer') throw new Error('provider batch authorization status is inaccurate')
