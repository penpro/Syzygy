...
| `frontend/src/workspace/scenarioLabelModel.ts` | Penumbra original | Namespaced immutable label and scenario-assignment events, exact-parent guards, canonical strict event hashes/readback, deterministic concurrent rename projection, filtering, and orphan/collision inspection |
...
| `frontend/src/workspace/suggestionAutomation.ts` | Penumbra original | Exact project/research-revision guarded suggestion proposal/decision mutations for semantic MCP automation without draft authority |
...
| `frontend/src/workspace/projectResearchEventAttestation.ts` | Penumbra original | Bounded parallel Yjs ledger for exact-hash registered-installation research-event signatures with a closed event-kind vocabulary, retained hash and author resolver validation, convergence, replay, and content-minimized inspection |
| `frontend/src/workspace/researchEventAttribution.ts` | Penumbra original | Scenario-turn, vote, annotation, label/assignment, and immutable policy-version adoption layer that locates/hashes exact committed product/MCP events, rejects cross-author claims, publishes best-effort registered-device attribution, and reports explicit unsigned fallback without rolling back research state |
...
| `frontend/src/workspace/policyVersionModel.ts` and `frontend/src/workspace/versionAutomation.ts` | Penumbra original | Exact semantic-editor snapshot adapter with document/head concurrency guards, immutable checkpoint output, and exact canonical-envelope attestation hash verification |
