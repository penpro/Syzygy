...
| `model_provider.rs` | Rust-owned remote-model HTTP/normalization boundary. The OpenAI Responses one-shot and SSE wire contracts, bounded timeout/stream bytes, stalled-body deadline, terminal sequencing, and in-flight/inter-event cancellation have fake-server evidence but are not product-wired pending credential integration and disclosure gates. |
