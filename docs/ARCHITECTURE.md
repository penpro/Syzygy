...
| `model_provider.rs` | Rust-owned remote-model HTTP/normalization boundary. OpenAI Responses and Anthropic Messages one-shot/SSE plus Gemini Interactions and xAI Responses one-shot wire contracts have fake-server evidence with bounded controls and sanitized normalization. |
...
| `provider_stream.rs` | Incremental provider SSE normalization. OpenAI and Anthropic decoders handle fragmented frames, text/usage/finish lifecycles, unknown future events, sanitized provider errors, and bounded malformed/truncated input. Anthropic private-thinking/tool delta bodies are not retained by the normalized stream. |
