...
| `frontend/src-tauri/src/provider_runtime.rs` | repository commit | Penumbra original / repository MIT | Fixed built-in endpoint dispatch, OS-vault lookup, cancellation registry, sanitized outcome, and Rust-authored content-free provider-run record. Generation remains unregistered pending disclosure UI. |
