...
| Remote provider silently stores research or transmits to an unsafe endpoint | OpenAI request fixture forces `store:false`, requires matching content disclosure, and accepts only HTTPS or literal loopback HTTP | Network streaming/tools and every additional adapter need equivalent wire inspection; provider policy must be rechecked at release |
| Fragmented or future provider stream corrupts research evidence | Incremental OpenAI decoder tests split UTF-8 at every byte, join multiline data, surface unknown types, order usage before finish, sanitize errors, and fail malformed/mismatched/oversized/truncated frames; the one-shot transport proves bounded timeout and idempotent cancellation through body collection | Live SSE dispatch/backpressure, retry/duplicate event semantics, tool arguments, and adversarial long-stream fuzzing remain open |
...
| Adversarial run record hides unfavorable evidence or spends more compute than its baseline | Typed validator requires every claim audit, known source snapshots, both planned judge orders, supported-minority retention, equal actual calls, finite accounting, and human acceptance before mutation | Typed input is not yet a public JSON schema; real orchestration, failure/cancellation accounting, corpus scoring, and statistical review remain open |
