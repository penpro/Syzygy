...
| `mcp_setup.rs` | Running-executable discovery, copy-ready setup, and explicit local/LAN Codex registration. It edits only named MCP tables in the user's shared Codex TOML, preserves unrelated settings through a formatting-preserving parser, and uses a recoverable same-directory replace. |
| `lan_dev_coordinator.rs` | App-owned Node coordinator lifecycle plus materialization of the authenticated loopback MCP attachment used by Codex; installed registration does not depend on repository scripts. |
...
- **MCP automation is semantic and live.** Document mutations require a read revision and fail
  ...
  MCP scenario votes additionally attempt an exact-event-hash installation attestation after the
  vote transaction. Research inspection validates the signature, registered-device participant
  claim, and live event hash but returns no public key/signature body. This is device continuity,
  not a person, organization, or Sybil-resistant voting claim.
