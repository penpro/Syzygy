...
| `frontend/src-tauri/src/model_provider.rs` | repository commit | Penumbra original / repository MIT | Disclosure-gated OpenAI Responses, Anthropic Messages, Gemini Interactions, and xAI Responses request/stream construction, provider-neutral normalization, xAI ZDR attestation, bounded deadlines/bytes, and caller-controlled cancellation. |
...
| Native adversarial batch authorization and reservation in `frontend/src-tauri/src/provider_runtime.rs` | repository commit | Penumbra original / repository MIT | Original exact-route/call/source scope validation, native content-free disclosure, random expiring in-memory capability, status/denial/revocation, and private atomic one-use route/total reservation boundary. No external product code, prompt, schema, or fixture was used. |
...
| `scripts/plugin-certifier.mjs`, its tests, and `examples/plugins/citation-auditor` | repository commit | Penumbra original / repository MIT | Bounded schema validation, real-path containment, proposal fixtures, authority probes, and an original executable zero-import signed reference package. |
| `frontend/src/extensions/pluginAuthorityBroker.ts` and tests | repository commit | Penumbra original / repository MIT | Original short-lived least-authority session broker for detached snapshots, pending proposals, and non-executing Drive/network/model decisions. No external product code, prompt, schema, or fixture was used. |
| `docs/wit/syzygy-research-plugin-v1.wit`, `frontend/src/extensions/pluginWasiContract.ts`, and tests | repository commit | Penumbra original / repository MIT | Original zero-import WIT world and bounded invocation/output validators. Design terminology was checked against the upstream Bytecode Alliance Component Model documentation; no external product code, WIT, prompt, schema, or fixture was copied. |
